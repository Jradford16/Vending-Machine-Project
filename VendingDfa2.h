...
public:
  ...
  // Primary handler that drives the motion of our underlying dfa.
  // Return values specify which state we are on.
  TokenProgress ProgressState(Alphabet token);

  // Called by parent automata to reset internal halted state back to start
  // as well as current item cost.
  void ResetSubsystem();
